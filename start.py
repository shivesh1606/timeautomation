...
def get_events_by_date(api_service, event_date):
    ...
    event_date = datetime(int(split_date[0]), int(
        split_date[1]), int(split_date[2]), 00, 00, 00, 0)
    # event_date = pytz.UTC.localize(event_date).isoformat()
    event_date = datetime.strftime(event_date, '%Y-%m-%dT%H:%M:%S.%fZ')
    end = datetime(int(split_date[0]), int(
        split_date[1]), int(split_date[2]), 23, 59, 59, 999999)
    end = datetime.strftime(end, '%Y-%m-%dT%H:%M:%S.%fZ')
    print(">>>>>>>>>>> Gettings events for date: ", event_date)
    events_result = api_service.events().list(calendarId='primary', timeMin=event_date,
...
def main(query, start_Date=None, end_Date=None):
    ...
    for event in events:
        try:
            ...
            try:
                ...
                event_start_date = event_start_date.astimezone(
                    pytz.timezone('Asia/Kolkata'))
                event_end_date = datetime.strptime(end, '%Y-%m-%dT%H:%M:%S%z')
                event_end_date = event_end_date.astimezone(
                    pytz.timezone('Asia/Kolkata'))
            except:
                event_start_date = datetime.strptime(start, '%Y-%m-%d')
                event_start_date = event_start_date.astimezone(
                    pytz.timezone('Asia/Kolkata'))
                event_end_date = datetime.strptime(end, '%Y-%m-%d')
                event_end_date = event_end_date.astimezone(
                    pytz.timezone('Asia/Kolkata'))
            ...
            try:
                ...
                event_discription = event['description']
            except Exception as e:
                pass
            try:
            ...
            event_dict = {
                ...
                "hangout_link": event_hangout_link


            }
            event_list = [title, creator, attendees, event_start_date, event_start_time,
                          event_end_date, event_end_time, location, event_discription, event_hangout_link]
    ...
    if query == 2:
        if os.path.isfile(os.path.join(path, "SavedCalendar_"+start_Date+".csv")):
            os.remove(os.path.join(path, "SavedCalendar_"+start_Date+".csv"))

        filepath = os.path.join(path, "SavedCalendar_"+start_Date+".csv")
    elif query == 3:
        if os.path.isfile(os.path.join(path, "SavedCalendar_"+start_Date+"_"+end_Date+".csv")):
            os.remove(os.path.join(path, "SavedCalendar_" +
                      start_Date+"_"+end_Date+".csv"))
        filepath = os.path.join(
            path, "SavedCalendar_"+start_Date+"_"+end_Date+".csv")
